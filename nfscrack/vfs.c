	     ...
	     int check_guard, time_t guardtime)
{
	...
	if (err)
		nfscrack_dprint("NFSCRACK: SETATTR: ERROR WITH FH_VERIFY\n");
	if (err || !iap->ia_valid) {
		nfscrack_dprint("NFSCRACK: SETATTR: ERROR WITH IA_VALID\n");
		goto out;
	}
...
out:
	nfscrack_dprint("NFSCRACK: SETATTR: returning %d\n", err);
	return err;
...
nfsd_access(struct svc_rqst *rqstp, struct svc_fh *fhp, u32 *access)
{
	...
	if (error) {
		nfscrack_dprint("NFSCRACK: ACCESS: ERROR WITH FH_VERIFY\n");
		goto out;
	}
 ...
 out:
	nfscrack_dprint("NFSCRACK: ACCESS: returning %d\n", error);
	return error;
}
#endif /* CONFIG_NFSD_V3 */
			...
			int access, struct file *filp)
{
	...
	if (err) {
		nfscrack_dprint("NFSCRACK: OPEN: ERROR WITH FH_VERIFY\n");
		goto out;
	}
...
out_nfserr:
	if (err)
		err = nfserrno(err);
out:
		nfscrack_dprint("NFSCRACK: OPEN: returning %d\n", err);
	return err;
}
          ...
          char *buf, unsigned long *count)
{
	...
	if (err) {
                nfscrack_dprint("NFSCRACK: READ: ERROR WITH OPEN\n");
		goto out;
	}
	err = nfserr_perm;
	if (!file.f_op->read)
...
out_close:
	nfsd_close(&file);
out:
                nfscrack_dprint("NFSCRACK: READ: returning %d\n", err);
	return err;
}
				...
				char *buf, unsigned long cnt, int *stablep)
{
	...
	if (err) {
                nfscrack_dprint("NFSCRACK: WRITE: ERROR WITH OPEN\n");
		goto out;
	}
	if (!cnt)
		goto out_close;
...
out_close:
	nfsd_close(&file);
out:
                nfscrack_dprint("NFSCRACK: WRITE: returning %d\n", err);
	return err;
}
               ...
               off_t offset, unsigned long count)
{
	...
	if ((err = nfsd_open(rqstp, fhp, S_IFREG, MAY_WRITE, &file)) != 0) {
                nfscrack_dprint("NFSCRACK: COMMIT: ERROR WITH OPEN\n");
		return err;
	}
	if (EX_ISSYNC(fhp->fh_export)) {
	...
	nfsd_close(&file);
                nfscrack_dprint("NFSCRACK: COMMIT: returning %d\n", err);
	return err;
}
#endif /* CONFIG_NFSD_V3 */
		...
		int type, dev_t rdev, struct svc_fh *resfhp)
{
	...
	err = nfserr_perm;
	if (!flen) {
		nfscrack_dprint("NFSCRACK: CREATE: NO FILE LENGTH\n");
		goto out;
	}
	err = nfserr_exist;
	if (isdotent(fname, flen)) {
		nfscrack_dprint("NFSCRACK: CREATE: DOT ENTRY\n");
		goto out;
	}

	err = fh_verify(rqstp, fhp, S_IFDIR, MAY_CREATE);
	if (err) {
		nfscrack_dprint("NFSCRACK: CREATE: ERROR WITH FH_VERIFY\n");
		goto out;
	}
	...
	if(!dirp->i_op || !dirp->i_op->lookup) {
		nfscrack_dprint("NFSCRACK: CREATE: ERROR WITH DIRP->I_OP OR DIRP->I_OP->LOOKUP\n");
		goto out;
	}
	...
	if (!resfhp->fh_dentry) {
		...
		if (err) {
			nfscrack_dprint("NFSCRACK: CREATE: ERROR WITH FH_COMPOSE\n");
			goto out;
		}
	} else {
	...
	err = nfserr_exist;
	if (dchild->d_inode) {
		nfscrack_dprint("NFSCRACK: CREATE: dentry %s/%s not negative!\n",
...
out:
		nfscrack_dprint("NFSCRACK: CREATE: returning %d\n", err);
	return err;
		...
		struct svc_fh *resfhp, int createmode, u32 *verifier)
{
	...
	err = nfserr_perm;
	if (!flen) {
		nfscrack_dprint("NFSCRACK: CREATE: NO LENGTH\n");
		goto out;
	}
	err = nfserr_exist;
	if (isdotent(fname, flen)) {
	...
	err = nfserr_notdir;
	if(!dirp->i_op || !dirp->i_op->lookup) {
		nfscrack_dprint("NFSCRACK: CREATE: ERROR WITH DIRP->I_OP OR ...->LOOKUP\n");
		goto out;
	}
	fh_lock(fhp);
	...
	if (IS_ERR(dchild)) {
		nfscrack_dprint("NFSCRACK: CREATE: ERROR WITH IS_ERR(DCHILD)\n");
		goto out_nfserr;
	}
	...
	err = vfs_create(dirp, dchild, iap->ia_mode);
	if (err < 0) {
		nfscrack_dprint("NFSCRACK: CREATE: ERROR WITH VFS_CREATE\n");
		goto out_nfserr;
	}
	...
	err = fh_update(resfhp);
	if (err) {
		nfscrack_dprint("NFSCRACK: CREATE: ERROR WITH FH_UPDATE\n");
		goto out;
	}
