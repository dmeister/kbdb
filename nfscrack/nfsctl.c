...
#ifdef NFSCRACK
int nfsc_read(char *page, char **start, off_t offset, 
...
int nfsc_write(struct file *file, const char *buffer,
	       unsigned long count, void *data) {
	int len;
	int rand_old = DBRANDSIZE;
	char *buf = "";
	MOD_INC_USE_COUNT;
	...
	DBRANDSIZE = simple_strtoul(buf, NULL, 10);
	if ((DBRANDSIZE%4 != 0) || (DBRANDSIZE < 4) || (DBRANDSIZE > 64))  {
		DBRANDSIZE = rand_old;
		goto error;
	}
...
#ifdef NFSCRACK
	struct proc_dir_entry *crackentry;
#endif
...
#ifdef NFSCRACK
	crackentry = create_proc_entry("fs/nfs/nfscrack", 0, NULL);
	if (crackentry) {
		crackentry->read_proc = nfsc_read;
		crackentry->write_proc = nfsc_write;
	}
#endif
}
...
nfsd_init(void)
{
	...
	printk(KERN_INFO "knfsd version: $Id: nfsctl.c,v 1.5 2004/01/23 17:17:42 avishay Exp $\n");
	printk(KERN_INFO "knfsd compiled: " __DATE__ " " __TIME__ "\n");
#ifdef MODULE
...
#ifdef NFSCRACK
	remove_proc_entry("fs/nfs/nfscrack", NULL);
#endif
