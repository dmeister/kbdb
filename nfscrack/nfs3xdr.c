...
#ifdef NFSCRACK 
static inline u32 *
nfscrack_encode_fh(u32 *p, struct svc_fh *fhp)
{
	int     ret;
	int	fhsize;
	int	size;
	...
	size = DBRANDSIZE;
	fhsize = fhp->fh_handle.fh_size;
	fhp->fh_handle.fh_size = DBRANDSIZE;

	nfscrack_dprint("ENTR nfscrack_encode_fh:\n");
	printhexstr((char *)fhp->fh_handle.fh_base.fh_pad, fhsize);
	nfscrack_dprint("\n");
	...
	*p++ = htonl(size);
	...
	keyfhr.data = fhp->fh_handle.fh_base.fh_pad;
	keyfhr.size = fhsize;

	if ((ret = dbfhrp->get(dbfhrp, NULL, &keyfhr, &datafhr, 0)) == 0) {
		memcpy(p, datafhr.data, datafhr.size);
		memcpy(fhp, datafhr.data, datafhr.size);
		if (DBRANDSIZE < fhsize)
			memset(&fhp->fh_handle.fh_base.fh_pad[DBRANDSIZE/4], 0, 64 - DBRANDSIZE);
	}
	else {
		nfscrack_dprint("NFSCRACK ERROR: nfscrack_encode_fh: getting fh unsuccessful\n");
	}

	nfscrack_dprint("EXIT nfscrack_encode_fh:\n");
	printhexstr((char *)fhp->fh_handle.fh_base.fh_pad, fhsize);
	nfscrack_dprint("\n");
